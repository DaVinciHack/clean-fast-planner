...
describe(createPublicOauthClient, () => {
  ...
  async function expectRedirectToMultipass(tokenPromise) {
    ...
    expect(Object.fromEntries(url.searchParams.entries())).toEqual(expect.objectContaining({
      ...
      scope: ["offline_access", ...(clientArgs.scopes ?? ["api:read-data", "api:write-data"].sort())].join(" "),
  ...
  describe.each([{
    ...
    it("calls common with expected values", () => {
      ...
      }), commonJs.createAuthorizationServer(clientArgs.ctxPath ?? "/multipass", clientArgs.foundryUrl), expect.any(Function), expect.anything(), expect.any(Function), undefined, (clientArgs.scopes ?? ["api:read-data", "api:write-data"]).sort().join(" "));
    });
    describe.each([{
      localStorage: {
        refresh_token: "a-refresh-token",
        requestedScopes: (clientArgs.scopes ?? ["api:read-data", "api:write-data"]).sort().join(" ")
      },
      sessionStorage: {}
    }, {
      localStorage: {},
