...
var chunkJEHRAVC5_cjs = require('./chunk-JEHRAVC5.cjs');
...
async function toDataValue(value, client) {
  ...
  if (isAttachmentUpload(value)) {
    const attachment = await chunkZV5KKQ4R_cjs.Attachment_exports.upload(client, value.data, {
      filename: value.name
    });
    return await toDataValue(attachment.rid, client);
  }
  if (isAttachmentFile(value)) {
    const attachment = await chunkZV5KKQ4R_cjs.Attachment_exports.upload(client, value, {
      filename: value.name
    });
    return await toDataValue(attachment.rid, client);
  }
  ...
  if (chunkJEHRAVC5_cjs.isWireObjectSet(value)) {
    return value;
  }
  if (chunkJEHRAVC5_cjs.isObjectSet(value)) {
    return chunkJEHRAVC5_cjs.getWireObjectSet(value);
  }
...
async function applyAction(client, action, parameters, options = {}) {
  const clientWithHeaders = chunkZV5KKQ4R_cjs.addUserAgentAndRequestContextHeaders(chunkJEHRAVC5_cjs.augmentRequestContext(client, (_) => ({
    finalMethodCall: "applyAction"
  })), action);
  if (Array.isArray(parameters)) {
    const response = await chunkZV5KKQ4R_cjs.Action_exports.applyBatch(clientWithHeaders, await client.ontologyRid, action.apiName, {
    ...
    const response = await chunkZV5KKQ4R_cjs.Action_exports.apply(clientWithHeaders, await client.ontologyRid, action.apiName, {
...
var GeotimeSeriesPropertyImpl = class {
  ...
  async getLatestValue() {
    const latestPointPromise = chunkZV5KKQ4R_cjs.TimeSeriesValueBankProperty_exports.getLatestValue(this.#client, await this.#client.ontologyRid, ...this.#triplet);
  ...
  async *asyncIterValues(query) {
    const streamPointsIterator = await chunkZV5KKQ4R_cjs.TimeSeriesValueBankProperty_exports.streamValues(this.#client, await this.#client.ontologyRid, ...this.#triplet, query ? {
...
var MediaReferencePropertyImpl = class {
  ...
  async fetchContents() {
    return chunkZV5KKQ4R_cjs.MediaReferenceProperty_exports.getMediaContent(this.#client, await this.#client.ontologyRid, ...this.#triplet, {
      preview: true
      // TODO: Can turn this back off when backend is no longer in beta.
    });
  }
  async fetchMetadata() {
    const r = await chunkZV5KKQ4R_cjs.MediaReferenceProperty_exports.getMediaMetadata(this.#client, await this.#client.ontologyRid, ...this.#triplet, {
      preview: true
      // TODO: Can turn this back off when backend is no longer in beta.
    });
    return {
...
var TimeSeriesPropertyImpl = class {
  ...
  async getFirstPoint() {
    return chunkZV5KKQ4R_cjs.TimeSeriesPropertyV2_exports.getFirstPoint(this.#client, await this.#client.ontologyRid, ...this.#triplet);
  }
  async getLastPoint() {
    return chunkZV5KKQ4R_cjs.TimeSeriesPropertyV2_exports.getLastPoint(this.#client, await this.#client.ontologyRid, ...this.#triplet);
  }
  ...
  async *asyncIterPoints(query) {
    const streamPointsIterator = await chunkZV5KKQ4R_cjs.TimeSeriesPropertyV2_exports.streamPoints(this.#client, await this.#client.ontologyRid, ...this.#triplet, query ? {
      range: getTimeRange(query)
    } : {});
    for await (const timeseriesPoint of asyncIterPointsHelper(streamPointsIterator)) {
      yield timeseriesPoint;
...
function createOsdkInterface(underlying, interfaceDef) {
  const [objApiNamespace] = chunkJEHRAVC5_cjs.extractNamespace(interfaceDef.apiName);
  return Object.freeze(Object.defineProperties({}, {
    ...
    ...Object.fromEntries(Object.keys(interfaceDef.properties).map((p) => {
      ...
      const [apiNamespace, apiName] = chunkJEHRAVC5_cjs.extractNamespace(p);
...
function get$link(holder) {
  ...
  return Object.freeze(Object.fromEntries(Object.keys(objDef.links).map((linkName) => {
    ...
    const value = !linkDef.multiplicity ? {
      fetchOne: (options) => chunkJEHRAVC5_cjs.fetchSingle(client, objDef, options ?? {}, chunkJEHRAVC5_cjs.getWireObjectSet(objectSet)),
      fetchOneWithErrors: (options) => chunkJEHRAVC5_cjs.fetchSingleWithErrors(client, objDef, options ?? {}, chunkJEHRAVC5_cjs.getWireObjectSet(objectSet))
    } : objectSet;
    ...
    if (propKey in objectDef.properties && typeof objectDef.properties[propKey].type === "string" && specialPropertyTypes.has(objectDef.properties[propKey].type)) {
      ...
      if (derivedPropertyTypeByName[propKey].type === "attachment") {
        if (Array.isArray(rawValue)) {
          rawObj[propKey] = rawValue.map((a) => chunkJEHRAVC5_cjs.hydrateAttachmentFromRidInternal(client, a.rid));
        } else {
          rawObj[propKey] = chunkJEHRAVC5_cjs.hydrateAttachmentFromRidInternal(client, rawValue.rid);
        }
      } else {
...
function createSpecialProperty(client, objectDef, rawObject, p) {
  ...
  if (propDef.type === "attachment") {
    if (Array.isArray(rawValue)) {
      return rawValue.map((a) => chunkJEHRAVC5_cjs.hydrateAttachmentFromRidInternal(client, a.rid));
    }
    return chunkJEHRAVC5_cjs.hydrateAttachmentFromRidInternal(client, rawValue.rid);
  }
...
async function loadActionMetadata(client, actionType) {
  const r = await chunkZV5KKQ4R_cjs.ActionTypeV2_exports.get(client, await client.ontologyRid, actionType);
  return generatorConverters.wireActionTypeV2ToSdkActionMetadata(r);
}
async function loadFullObjectMetadata(client, objectType) {
  const full = await chunkZV5KKQ4R_cjs.ObjectTypeV2_exports.getFullMetadata(client, await client.ontologyRid, objectType, {
    preview: true
  });
...
async function loadInterfaceMetadata(client, objectType) {
  const r = await chunkZV5KKQ4R_cjs.OntologyInterface_exports.get(client, await client.ontologyRid, objectType, {
    preview: true
  });
...
async function loadQueryMetadata(client, queryTypeApiNameAndVersion) {
  ...
  const r = await chunkZV5KKQ4R_cjs.QueryType_exports.get(client, await client.ontologyRid, apiName, {
    version
  });
...
var USER_AGENT = `osdk-client/${"2.3.0-beta.2"}`;

// src/createMinimalClient.ts
function createMinimalClient(metadata, baseUrl, tokenProvider, options = {}, fetchFn = global.fetch, objectSetFactory = chunkJEHRAVC5_cjs.createObjectSet, createOntologyProviderFactory = createStandardOntologyProviderFactory) {
  if (process.env.NODE_ENV !== "production") {
    try {
      new URL(baseUrl);
    } catch (e) {
...
async function applyQuery(client, query, params) {
  ...
  const response = await chunkZV5KKQ4R_cjs.Query_exports.execute(chunkZV5KKQ4R_cjs.addUserAgentAndRequestContextHeaders(chunkJEHRAVC5_cjs.augmentRequestContext(client, (_) => ({
...
async function remapQueryResponse(client, responseDataType, responseValue, definitions) {
  ...
  switch (responseDataType.type) {
    ...
    case "attachment": {
      return chunkJEHRAVC5_cjs.hydrateAttachmentFromRidInternal(client, responseValue);
    }
    case "object": {
    ...
    case "objectSet": {
      ...
      if (typeof responseValue === "string") {
        return chunkJEHRAVC5_cjs.createObjectSet(def, client, {
          type: "intersect",
          objectSets: [{
            type: "base",
      ...
      return chunkJEHRAVC5_cjs.createObjectSet(def, client, responseValue);
    }
    case "struct": {
...
function createClientInternal(objectSetFactory, baseUrl, ontologyRid, tokenProvider, options = void 0, fetchFn = fetch) {
  ...
  const clientCtx = createMinimalClient({
    ...
    ...options,
    logger: options?.logger ?? new chunkJEHRAVC5_cjs.MinimalLogger()
        ...
        case unstable.__EXPERIMENTAL__NOT_SUPPORTED_YET__fetchOneByRid.name:
          return {
            fetchOneByRid: async (objectType, rid, options2) => {
              return await chunkJEHRAVC5_cjs.fetchSingle(clientCtx, objectType, options2, createWithRid([rid]));
            }
          };
        case unstable.__EXPERIMENTAL__NOT_SUPPORTED_YET__createMediaReference.name:
          return {
            createMediaReference: async (args) => {
              ...
              return await chunkZV5KKQ4R_cjs.MediaReferenceProperty_exports.upload(clientCtx, await clientCtx.ontologyRid, objectType.apiName, propertyType, data, {
        ...
        case unstable.__EXPERIMENTAL__NOT_SUPPORTED_YET__fetchPageByRid.name:
          return {
            fetchPageByRid: async (objectOrInterfaceType, rids, options2 = {}) => {
              return await chunkZV5KKQ4R_cjs.fetchPage(clientCtx, objectOrInterfaceType, options2, createWithRid(rids));
            }
          };
      }
  ...
  const client = Object.defineProperties(clientFn, {
    [chunkZV5KKQ4R_cjs.symbolClientContext]: {
      value: clientCtx
    },
    [symbolClientContext2]: {
      value: clientCtx
    },
    [chunkZV5KKQ4R_cjs.additionalContext]: {
      value: clientCtx
    },
    fetchMetadata: {
      value: fetchMetadata
...
var createClient = createClientInternal.bind(void 0, chunkJEHRAVC5_cjs.createObjectSet);
