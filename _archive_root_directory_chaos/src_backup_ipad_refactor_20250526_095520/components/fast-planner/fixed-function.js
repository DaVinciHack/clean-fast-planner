  ...
  const generateStopCardsData = (waypoints, routeStats, selectedAircraft, weather = { windSpeed: 0, windDirection: 0 }) => {
    ...
    const taxiFuelValue = taxiFuel || 50;
    const reserveFuelValue = reserveFuel || 600;
    
    // Calculate intermediate stops (for deck fuel)
    const intermediateStops = Math.max(0, waypoints.length - 2);
    const deckTimeHours = (intermediateStops * (deckTimePerStop || 5)) / 60; // Convert from minutes to hours
    const deckFuelValue = Math.round(deckTimeHours * (deckFuelFlow || 400));
    
    // Calculate contingency fuel
    const contingencyFuelValue = Math.round((totalTripFuel * (contingencyFuelPercent || 10)) / 100);
    ...
    for (let i = 0; i < legDetails.length; i++) {
      ...
      const remainingDeckTimeHours = (remainingIntermediateStops * (deckTimePerStop || 5)) / 60;
      const remainingDeckFuel = Math.round(remainingDeckTimeHours * (deckFuelFlow || 400));
      ...
      if (selectedAircraft) {
        ...
        maxPassengers = Math.floor(usableLoad / (passengerWeight || 220));
      ...
      const maxPassengersWeight = isFinalDestination ? null : (maxPassengers * (passengerWeight || 220));
