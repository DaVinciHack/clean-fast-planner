...
class MapInteractionHandler {
      ...
      try {
        ...
        routeLayers.forEach(layer => {
          if (map.getLayer(layer)) {
            ...
            // Hover events for cursor change
    ...
    startRouteLayerMonitor(map) {
      ...
      this.routeLayerMonitor = setInterval(() => {
        ...
        if (routeRelatedLayers.length > 0) {
          ...
          // Only stop monitoring if we actually found route layers with drag handlers
          if (foundLayers.length > 0) {
            console.log(`🎯 STOPPING MONITOR - Found ${foundLayers.length} route layers with drag handlers`);
            clearInterval(this.routeLayerMonitor);
            this.routeLayerMonitor = null;
          } else {
            console.log(`🔍 CONTINUING MONITOR - No route layers found yet (check ${checkCount})`);
          }
    ...
    handleRouteDragComplete(insertIndex, coords, dragData = {}) {
      ...
      try {
        ...
        // Simple waypoint addition at the drag end coordinates
        const nameToUse = isWaypointMode ? `Waypoint ${insertIndex + 1}` : `Stop ${insertIndex + 1}`;
        const pointOptions = { isWaypoint: isWaypointMode, type: isWaypointMode ? 'WAYPOINT' : 'STOP', pointType: isWaypointMode ? 'NAVIGATION_WAYPOINT' : 'LANDING_STOP' };
        
        this.waypointManager.addWaypointAtIndex(coords, nameToUse, insertIndex, pointOptions);
        
        if (window.LoadingIndicator) {
          window.LoadingIndicator.updateStatusIndicator(`Added ${nameToUse}`, 'success', 2000);
        }
        
      } catch (error) {
        console.error('MapInteractionHandler: Error in handleRouteDragComplete:', error);
        if (window.LoadingIndicator) {
          window.LoadingIndicator.updateStatusIndicator('Error adding waypoint from drag', 'error', 2000);
        }
      } finally {
        setTimeout(() => {
          window._processingRouteDrag = false;
    ...
    // Get waypoint handler for mode handling
    getWaypointHandler() {
      return window.waypointHandler || null;
    }
