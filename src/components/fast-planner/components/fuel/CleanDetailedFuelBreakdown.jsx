...
const CleanDetailedFuelBreakdown = ({
  ...
  const getPassengerRequest = useCallback((stopName, requestType) => {
    ...
    if (passengerRequests[key] !== undefined) {
      const value = passengerRequests[key];
      return (value && value > 0) ? value : ''; // Only show real numbers, hide zeros
    }
    ...
    if (wizardData[legKey]) {
      if (requestType === 'passengerCount') {
        const value = wizardData[legKey].passengerCount;
        return (value && value > 0) ? value : ''; // Only show real numbers, hide zeros
      } else if (requestType === 'totalWeight') {
        const value = wizardData[legKey].totalWeight;
        return (value && value > 0) ? value : ''; // Only show real numbers, hide zeros
      }
    }
    
    return ''; // Return empty string for zero values - better iPad UX
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (overloadedStops.length > 0 && !hasTriggeredOptimization) {
        ...
        const flightConfiguration = {
          ...
          routeStats,
          alternateRouteData,
          // 🚀 CRITICAL: Pass alternate split point for improved optimization
          alternateSplitPoint: alternateRouteData?.splitPoint || null,
        ...
        // 🚀 DEBUG: Check alternate split point data
        console.log('🚀 ALTERNATE SPLIT POINT DEBUG:', {
          alternateRouteData,
          alternateSplitPoint: alternateRouteData?.splitPoint,
          hasValidSplitPoint: !!(alternateRouteData?.splitPoint?.lat && alternateRouteData?.splitPoint?.lng)
        });
        ...
        console.log('🎯 OPTIMIZATION WILL PRIORITIZE:', {
          splitPointProvided: !!flightConfiguration.alternateSplitPoint,
          splitPointCoords: flightConfiguration.alternateSplitPoint,
          waypointsCount: flightConfiguration.waypoints.length,
          platformsCount: flightConfiguration.availablePlatforms.length
        });
          ...
          {fuelStopPopup.type === 'suggestions' && (
            <>
              ...
              <div className="ai-message">
                {(() => {
                  // Extract actual overload data from current stops
                  let overloadInfo = { locationName: 'your destination', excessWeight: 0 };
                  
                  // Check overloadedStops for actual data
                  if (overloadedStops.length > 0) {
                    const firstOverload = overloadedStops[0];
                    const locationName = firstOverload.name || firstOverload.stopName;
                    const requestedWeight = getPassengerRequest(locationName, 'totalWeight');
                    const availableWeight = firstOverload.availableWeight || 0;
                    const excessWeight = Math.max(0, requestedWeight - availableWeight);
                  ...
                  return (
                    <>
                      Weight overload detected at <span className="highlight">{overloadInfo.locationName}</span> - you'll be <span className="highlight">{overloadInfo.excessWeight} lbs over capacity</span>. Found <span className="highlight">{fuelStopPopup.data.suggestions?.length || 0} fuel stops</span> that can help.
                    </>
                  );
                })()}
              </div>
