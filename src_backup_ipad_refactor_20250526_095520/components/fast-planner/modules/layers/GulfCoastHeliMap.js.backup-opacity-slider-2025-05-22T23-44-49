...
class GulfCoastHeliMap {
  ...
  setOpacity(opacity) {
    try {
      const map = this.mapManager.getMap();
      if (!map) {
        console.warn('GulfCoastHeliMap.setOpacity: Map not available');
        return;
      }
      ...
      if (map.getLayer(this.layerId)) {
        map.setPaintProperty(this.layerId, 'raster-opacity', clampedOpacity);
        console.log(`GulfCoastHeliMap: Opacity set to ${clampedOpacity}`);
      }
      
      // Also handle the alternative layer if it exists
      if (map.getLayer(this.layerId + '-alt')) {
        map.setPaintProperty(this.layerId + '-alt', 'raster-opacity', clampedOpacity);
        console.log(`GulfCoastHeliMap: Alternative layer opacity set to ${clampedOpacity}`);
      }
    } catch (error) {
      console.error('Error setting Gulf Coast Helicopter Map opacity:', error);
    }
  }
  ...
  getOpacity() {
    try {
      const map = this.mapManager.getMap();
      if (!map) return 0.85; // Default opacity
      
      // Get opacity from the main layer first
      if (map.getLayer(this.layerId)) {
        const opacity = map.getPaintProperty(this.layerId, 'raster-opacity');
        return opacity !== undefined ? opacity : 0.85;
      }
      
      // Try alternative layer
      if (map.getLayer(this.layerId + '-alt')) {
        const opacity = map.getPaintProperty(this.layerId + '-alt', 'raster-opacity');
        return opacity !== undefined ? opacity : 0.85;
      }
      
      return 0.85; // Default opacity
    } catch (error) {
      console.error('Error getting Gulf Coast Helicopter Map opacity:', error);
      return 0.85;
    }
  }
}
